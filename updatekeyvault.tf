...
kv_identity_refs = toset(compact(flatten([
	for kv in concat(values(local.key_vaults_common_to_deploy_iterator), values(local.key_vaults_env)) :
		[ for policy in values(lookup(kv, "access_policies", {})) : try(policy.identity_ref, null)]
])))
}
	...
	dynamic "access_policy" {
		...
		content {
			...
			object_id = try(access_policy.value.object_id, data.azuread_service_principal.kv_identity_refs[access_policy.value.identity_ref].object_id)
				# Fallback to trying to resolve the object ID from the kv_identity_refs list if an identity_ref is provided but object_id is not (handles cases where the reference is used in multiple vaults and thus cant be resolved at the time of vault creation)
			key_permissions = access_policy.value.key_permissions
			secret_permissions = access_policy.value.secret_permissions
			certificate_permissions = access_policy.value.certificate_permissions
		}
...
data "azuread_service_principal" "kv_identity_refs" {
	for_each = local.kv_identity_refs
