					...
					logs_loga_targets = [ for k, v in data.azurerm_monitor_diagnostic_categories.env[type_key].logs : v 
						if 
							(
								...
								&& contains(keys(local.diag_logging_loga_targets.cognitive_accounts.per_resource.ai-foundry-eastus2), resource_value.name)
								&& contains(local.diag_logging_loga_targets.cognitive_accounts.per_resource.ai-foundry-eastus2[resource_value.name].loga_logs, k)
							)
							|| (
								type_key != "cognitive_accounts"
								&&	(
